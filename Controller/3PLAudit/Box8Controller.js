...
module.exports = {
  ...
  box8AnswerORG: async (req, res) => {
    try {
      const box1Answer = await Box8Model.find({ owner: req.org._id });
      res.status(200).json({
        success: true,
        box1Answer,
      });
  ...
  deleteForm: async (req, res) => {
    try {
      ...
      const box1 = await Box8Model.findById(req.params.id);
      if (!box1) {
        return res.status(400).json({
          success: false,
      ...
      await Box8Model.findByIdAndRemove(req.params.id);

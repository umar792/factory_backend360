...
module.exports = {
  ...
  box2AnswerORG: async (req, res) => {
    try {
      const box1Answer = await Box2Model.find({ owner: req.org._id });
      res.status(200).json({
        success: true,
        box1Answer,
      });
  ...
  deleteForm: async (req, res) => {
    try {
      ...
      const box1 = await Box2Model.findById(req.params.id);
      if (!box1) {
      ...
      await Box2Model.findByIdAndRemove(req.params.id);

...
module.exports = {
  ...
  CreateBox7: async (req, res) => {
    try {
      ...
      await Box7Model.create(req.body);
      res.status(200).json({
        success: true,
  ...
  box7Answer: async (req, res) => {
    try {
      if (req.user.role === "superAdmin") {
        ...
        const box1Answer = await Box7Model.find({ user: req.user._id });
        res.status(200).json({
          success: true,
          box1Answer,
        });
  ...
  box7AnswerORG: async (req, res) => {
    try {
      const box1Answer = await Box7Model.find({ owner: req.org._id });
      res.status(200).json({
        success: true,
        box1Answer,
      });
  ...
  deleteForm: async (req, res) => {
    try {
      ...
      const box1 = await Box7Model.findById(req.params.id);
      if (!box1) {
        return res.status(400).json({
          success: false,
      ...
      await Box7Model.findByIdAndRemove(req.params.id);

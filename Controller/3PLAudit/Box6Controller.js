...
module.exports = {
  ...
  box6Answer: async (req, res) => {
    try {
      if (req.user.role === "superAdmin") {
        ...
        const box1Answer = await Box6Model.find({ user: req.user._id });
        res.status(200).json({
          success: true,
          box1Answer,
        });
  ...
  box6AnswerORG: async (req, res) => {
    try {
      const box1Answer = await Box6Model.find({ owner: req.org._id });
      res.status(200).json({
        success: true,
        box1Answer,
      });
  ...
  deleteForm: async (req, res) => {
    try {
      ...
      const box1 = await Box6Model.findById(req.params.id);
      if (!box1) {
        return res.status(400).json({
          success: false,
      ...
      await Box6Model.findByIdAndRemove(req.params.id);
